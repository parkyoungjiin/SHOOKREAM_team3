...
<!--   </div> -->
  ...
  <div class="w3-row">
     ...
     <input type="hidden" id="product_idx${status.index }" value="${productSaleList.product_idx }">
   <div class="w3-col l3 s6">
      <div class="w3-container" style="padding-top: 100px;">
        <div class="w3-display-container">
          <img src="./upload/${productSaleList.image_main_file }" onError="this.onerror=null; this.src='resources/images/noImg.JPG';"  alt="..." style="width: 100%">
...
<%-- 									<button class="w3-button w3-black" onclick="location.href='ProductInfoForm.po?product_idx=${productSaleList.product_idx }&member_idx=${sessionScope.member_idx }'"> --%>
								...
								<div class="w3-display-topright">
									<c:set var="isWished" value="false" />
								 	<c:forEach var="wish" items="${wish}">
									   <c:if test="${wish.product_idx eq productSaleList.product_idx}">
							...
							<p id="product_brand" onclick="location.href='BrandCG.ma?cg=${productSaleList.product_brand }'" style="cursor: pointer;">${productSaleList.product_brand }</p>
							<p id="product_name" onclick="location.href='ProductInfoForm.po?product_idx=${productSaleList.product_idx }'" style="cursor: pointer;">${productSaleList.product_name }<br></p>
...
<%-- 									<c:out value="${discounted_price}" /> --%>
							...
							<div id="etcInfo" style="padding-bottom: 20px;">
								...
								<span>
									<i class="fa-solid fa-heart w3-grayscale" style="color:pink;"></i> ${productSaleList.product_wishcount }
								</span>
							</div>
						</div>
					</div>
	...
	<div class="paging">
        <c:choose>
			<c:when test="${param.pageNum > 1}">
				<a href="Sale.ma?pageNum=${param.pageNum - 1 }">이전</a>
			</c:when>
			<c:otherwise>
		...
		<c:choose>
			<c:when test="${param.pageNum < pageInfo.maxPage}">
				<a href="Sale.ma?pageNum=${param.pageNum + 1 }">다음</a>
			</c:when>
			<c:otherwise>
